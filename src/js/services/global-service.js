...
export default{
	//判断当前用户信息是否登录
	// isLogin() {
	// 	if(this.getLoginUserInfo().numi_auth){
	// 		return true;
	// 	}
     //    return false;
    // },
    ...
    //设置用户信息
    setUserInfo({tenancyName, numi_auth, expiredTime = -1}) {
    	if(expiredTime > 0) {
    		const _site_local_storage = app.getSiteLocalStorage();
			...
			Object.assign(_site_local_storage.userInfo, {tenancyName, numi_auth, expiredTime, version: app.Config.innerVersion});
    		app.utils.localStorage("siteLocalStorage", JSON.stringify(_site_local_storage));
    	} else {
